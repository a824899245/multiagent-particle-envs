...
class SDVN(object):
    ...
    def __init__(self, vehicle_num=100, control_num = 4, channel_num = 4):
        ...
        input = "envs/rural_small_"+str(vehicle_num)+".mobility.tcl"
        vehicle_num = 100
    ...
    def step(self, action_1, action_2, cnt):
        ...
        time = self.start_time + self.time_slot_duration / 1000.0 * (cnt+1)
        ...
        # reward = np.array([(self.ini_Aoi_sum - self.current_AoI.sum())/10000 , thr_sum/self.vehicle_num/4199765 ])
        reward = np.array([(self.ini_Aoi_sum - self.current_AoI.sum())/100 , thr_sum/100000000 ])
        # print(self.AoI_sum)
        # print(self.throughput_sum)
        # print(self.channel_penalty)
        # print(self.power_penalty)
    ...
    def eval(self, action_1, action_2, cnt):
        ...
        position_state = np.array(self.node_id_position)


        next_state = np.concatenate((position_state, control_state))
        next_state = self.current_state.reshape((self.control_num+self.vehicle_num)*3)
        next_state = np.concatenate((next_state, current_AoI))
        ...
        return [(ini_Aoi_sum - current_AoI.sum())/10000 , thr_sum/self.vehicle_num/4199765]
      

    def l_reset(self,cnt):
        '''
            reset the location of the submarine
        '''
        self.start_time = 100
