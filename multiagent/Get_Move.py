...
from .Global_Par import update_period
from .Init import init_controller,get_communication_node,init_node
...
def update_node_position(movement_matrix, node_position, start_t, update_period, animation, nodelist, com_nodelist,
    ...
    for i in range(0, int(1.0 / update_period)):
        node_position[:, 2] = node_position[:, 2] + speed_x * update_period
        node_position[:, 3] = node_position[:, 3] + speed_y * update_period
        node_id_position = node_position[:, [1, 2, 3]]
        if nodelist == [] or com_nodelist == []:
            nodelist.extend(init_node(node_id_position, controller))
            com_nodelist.extend(get_communication_node(node_id_position.shape[0]))
...
def control_movement(animation):
    ...
    nodelist = []
    com_nodelist = []
    controller = init_controller(node_num)
